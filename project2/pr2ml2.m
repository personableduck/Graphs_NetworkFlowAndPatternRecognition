...
img_lim=200;%Put adaboost Image Limit 10000 - each
%{
for img_t=1:2
    count=0;
for img=1:img_lim
...
load 'value_face.mat';
load 'valu_nonface.mat';

intensity_diff_face=value_face;
intensity_diff_nonface=value_nonface;
disp('both matrices loaded.');
%%%Plotting the histogram
%hist(intensity_diff(2,:));
sz=size(intensity_diff_face(:,1));
disp('started threshold calculation.');
disp('doing threshold calculation..');
...
disp('done with thresholds & polarity calculation.');
%Run Adaboost Code
...
min_error_index=[];
beta=[];
disp('started adaboosting.');
disp('doing adaboosting..');
T_lim=2;% Put Adaboost T limit ---> 101 Just in case;
...
disp('done with Adaboosting.');
 h=min_error_index;
beta;

disp('strated with super classification');
%Super classifier classifications: 
%just for faces
disp('doing super classification..');
for i=1:img_lim_face
    left=0;
    right=0;
    if mod(i,100)==0
...
disp('Done with super classification..');
