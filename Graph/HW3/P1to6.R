...
lp_op1 <- label.propagation.community(undirected_g)
...
# Problem 4
largest_community <- which.max(sizes(fg_op2.object))
sub_community_network <- induced.subgraph(undirected_g_option2, which(membership(fg_op2.object) == largest_community))
...
# Problem 6
ppr <- page.rank (g, algo = "prpack", vids = V(g)$name, directed = TRUE, damping = 0.85, personalized = TRUE, weights = E(g)$weight, options = NULL)
ppr[[1]]
